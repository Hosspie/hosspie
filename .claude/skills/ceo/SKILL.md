...
CEO가 다루는 것과 다루지 않는 것을 명확히 구분한다. 이 경계를 넘으면 CEO는 "전략"이 아니라 "구현 관리자"가 되어버린다.
...
이 영역은 **별도 실행 스킬**(예: gsd, frontend-developer, backend-developer 등)이나 사용자의 태스크 관리 도구에서 다룬다. CEO 스킬의 지속 문서(`vision.md`, `roadmap.md`)에는 Phase 번호, 상세 태스크, 진행 체크리스트를 기록하지 않는다.

**원칙**: "마일스톤까지는 CEO가, 그 아래는 실행 영역이 한다."
