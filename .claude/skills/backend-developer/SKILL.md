...
description: NestJS GraphQL API, Prisma 스키마, 데이터베이스 관리 가이드. API 엔드포인트 구현, 데이터베이스 스키마 변경, 마이그레이션, Prisma 쿼리 작성, NestJS 모듈/서비스/리졸버 작성, GraphQL 타입 정의, 타입 시스템 관리 작업 시 사용. 트리거: API, GraphQL, 리졸버, 서비스, Prisma, 데이터베이스, 스키마, 마이그레이션, 모듈, DTO, Input, ObjectType, codegen, 타입 시스템.
---
